node {
    docker.image('bbrfkr0129/build_test:with_boto').inside("-v /var/jenkins_home/") {
        ...
        sh 'cd iac-demo-cd && ansible-playbook -i hosts playbook/create-test-instance.yaml'
        sh 'cd iac-demo-cd && ec2.py --refresh-cache > /dev/null'
        sh 'cd iac-demo-cd && ansible-playbook -i ec2.py playbook/wait-for-instances-up.yaml'
    }
